...
def main(): Unit = {
  ...
  fileContent match {
    ...
    case Some(text) =>
      val splitText = splitIntoSingleWords(stringContent = text)

      splitText match {
        case None => println("The given string is an empty value.")
        case Some(afterSplit) =>
          val wordCounts = mapAllWordsLength(allWords = afterSplit)

          wordCounts match {
            case None => println("Cannot count the words.")
...
def mapAllWordsLength(allWords: Array[String]): Option[Unit] = {
  if (allWords.isEmpty) {
    None
  } else {
